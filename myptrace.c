...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            // if (!strncmp(callname(REG(regs)), "openat", 6))
            // {
            //     // sleep(1);
            // }
            // sleep(1);
            // PT_SYSEMU_SINGLESTEP
            // ptrace(PT_SYSEMU_SINGLESTEP, chld, NULL, NULL);

            printf("[RUN]\tSYSEMU\n");
            ptrace(PTRACE_SYSEMU, chld, NULL, NULL);
            wait_trap(chld);

            printf("[RUN]\tGETREGS\n");
            ptrace(PTRACE_GETREGS, chld, NULL, &regs);

            printf("system call %s from pid %d\n", callname(regs.user_syscall_nr), chld);
            ...
            if (regs.user_syscall_nr == SYS_openat)
            {
                ...
                //printf("openat %s %s %s %s %s!\n", (char *)regs.user_arg0, (char *)regs.user_arg1, (char *)regs.user_arg2, (char *)regs.user_arg3, (char *)regs.user_arg4);
            }
            ...
            // ptrace(PTRACE_SYSEMU_SINGLESTEP, chld, NULL, NULL);
            // wait_trap(chld);

            // ptrace(PTRACE_SYSEMU, chld, 0, 0);
            // wait_trap(chld);
            // PTRACE_SYSCALL
        }
    }
